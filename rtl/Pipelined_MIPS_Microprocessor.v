...
// 5-Stage Pipelined MIPS Processor with Hazard Unit
// Stages: IF -> ID -> EX -> MEM -> WB
// Features: Data Forwarding, Load-Use Hazard Stalling, Control Hazard Flushing
// =============================================================================
...
module Pipelined_MIPS_Microprocessor #(
    ...
    // Hazard detection: determine whether instruction uses Rs and/or Rt as source operands
    ...
    Branch_Unit branch_unit_inst (
        .opcode(InstrE[31:26]),
        .rt(InstrE[20:16]),
        .rs_value(SrcAE_forwarded),
        .zero(zeroE),
        .Branch(BranchE),
        .branch_taken(branch_taken_E)
    );

    // Branch target calculation: PCPlus4E + (SignImmE << 2)
    wire [31:0] beqshiftoutE = SignImmE << 2;
    wire [31:0] PCBranchE;
    Adder_32_bits addbranch_inst (
        .out(PCBranchE),
        .in1(beqshiftoutE),
        .in2(PCPlus4E)
    );

    // Jump target calculations
    wire [31:0] PCJumpE = {PCPlus4E[31:28], InstrE[25:0], 2'b00};
    wire [31:0] PCJrE   = SrcAE_forwarded;

    // Next PC selection and Control Hazard detection in EX stage
    always @(*) begin
        if (branch_taken_E) begin
            PCNext = PCBranchE;
            PCSrcE = 1'b1;
        end else if (JumpE == 2'b01) begin // j, jal
            PCNext = PCJumpE;
            PCSrcE = 1'b1;
        end else if (JumpE == 2'b10) begin // jr, jalr
            PCNext = PCJrE;
            PCSrcE = 1'b1;
        end else begin
            PCNext = PCPlus4F;
    ...
    // =========================================================================
    // ALUOutW already carries ALU results, HI, LO, or PCPlus4.
    // Loads select ReadDataW from memory.
