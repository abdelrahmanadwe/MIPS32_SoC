...
module tb_hw_interrupt;
    ...
    initial begin
        ...
        #200;
        $display("[TB] Asserting hardware_interrupt at t=%0t ns, PCF=%h", $time, MIPS.PCF);
    ...
    always @(posedge clock) begin
        if (MIPS.cp0_inst.exception_trigger && MIPS.cp0_inst.exception_cause == 32'h0000_0000) begin
            $display("[TB] SUCCESS: Hardware Interrupt triggered in CP0 at t=%0t ns! exception_epc=%h, Cause_in=%h",
                $time, MIPS.cp0_inst.exception_epc, MIPS.cp0_inst.exception_cause);
            interrupt_detected <= 1;
        end
        if (interrupt_detected && MIPS.PCF == 32'h0000_8180) begin
            $display("[TB] SUCCESS: At handler entry 0x0000_8180 at t=%0t ns: EPC_out=%h, Cause_out=%h",
                $time, MIPS.cp0_inst.EPC_out, MIPS.cp0_inst.Cause_out);
        end
    end
    ...
    initial begin
        ...
        #10000;
        if (interrupt_detected && MIPS.registers.registers[16][15:0] == 16'hD08E) begin
            $display("\n=======================================================");
            ...
            $display("    interrupt_detected=%b, $s0=%h", interrupt_detected, MIPS.registers.registers[16]);
            $display("=======================================================\n");
        end
        $finish;
    end
